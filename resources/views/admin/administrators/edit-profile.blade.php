...
<div class="card shadow mb-4 mt-2 border-bottom-main">
    ...
    <div class="card-body">
        <form enctype="multipart/form-data" class="form-course needs-validation" id="editProfileForm" novalidate
            method="POST" action={{route('admin.updateAdminProfile')}} autocomplete="off" role="form">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-6 col-md-12">
                            ...
                            <input type="text" class="form-control {{$errors->has('name') ? 'is-invalid' :'' }}"
                                value="{{!empty($user) ? $user->name: ''}}" id="name" name="name">
                        </div>
                        <div class="form-group col-lg-6 col-md-12">
                            <label for="email" class="col-form-label">Email:</label>
                            <input type="email" class="form-control {{$errors->has('email') ? 'is-invalid' :'' }}"
                                value="{{!empty($user) ? $user->email: ''}}" id="email" name="email">
...
<script>
    document.querySelector('#editProfileForm').addEventListener('submit', function (e) {
            ...
            axios.put(this.action, {
                ...
                .then((response) => {
                    let changed_pass = response.data.changed_pass;
                    enableSubmit(btn, 'Actualizar');
                    clearErrors();
                    console.clear();
                    (changed_pass ? document.querySelector('._alert-warning').remove() : '')
