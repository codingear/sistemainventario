...
@section('content')
    ...
    <form enctype="" id="editProductForm" class="needs-validation" method="PUT"
          action="{{route('productos.update',$product)}}" autocomplete="off" role="form">
        @csrf
        @method('PUT')
        <div class="row">
            ...
            <div class="col d-flex align-items-stretch img-principal">
                <div class="card shadow mb-4 mt-2 border-bottom-main w-100">
                    ...
                    <div class="card-body mb-2">
                        <img
                            src="{{ empty($product->image) ? 'https://dummyimage.com/900x900/f8f9fc/919191.png&text=Sin+Imagen' : $product->image->url }}"
                            class="img__principal"
                            alt="" id="principal_image">
                        <!-- <p class="text-center align-middle"> -->
                        <input type="text" class="invisible" name="principal_image"
                               value="{{ empty($product->image) ? '' : $product->image->id}}" id="principal_image_field"
                               hidden>
                        <!-- </p> -->
                    </div>
                    ...
                    <div id="gallery_select" class="container__imgs grid">
                        @foreach($product->images as $image)
                            <div class='item container_item'>
                                <img src="{{$image->url}}" class=thumbnail alt="{{$image->text_alter}}"
                                     data-src="{{$image->url}}">
                                <input type=text id='gallery_images' name='gallery_photos[]' value="{{$image->id}}"
                                       hidden>
