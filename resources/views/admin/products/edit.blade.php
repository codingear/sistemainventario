...
<form enctype="" id="editProductForm" class="needs-validation" method="PUT" action="{{route('productos.update',$product)}}" autocomplete="off" role="form">
    @csrf
    @method('PUT')
    <div class="row">
                ...
                <div class="card-body mb-2">
                    <div class="form-row">
                        ...
                        <div class="form-group col-lg-4 col-md-12">
                            ...
                            <input type="text" class="form-control" value="{{old('sku', $product->sku)}}" id="sku" name="sku">
                        </div>
                        <div class="form-group col-lg-4 col-md-12 {{$errors->has('category_id') ? 'is-invalid' :'' }}">
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-4 col-md-12">
                            ...
                            <input type="text" class="form-control" value="{{old('sale_price', $product->sale_price)}}" id="sale_price" name="sale_price">
                        </div>
                        <div class="form-group col-lg-4 col-md-12">
                            <label for="stock" class="col-form-label">Stock:</label>
                            <input type="number" class="form-control" value="{{old('stock', $product->stock)}}" id="stock" name="stock">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-lg-12 col-md-12">
                            <label for="description" class="col-form-label">Descripción</label>
                            <textarea class="form-control" id="description" rows="4" name="description">{{old('description', $product->description)}}</textarea>
                        </div>
                    </div>
                </div>
        ...
        <div class="col d-flex align-items-stretch img-principal">
            <div class="card shadow mb-4 mt-2 border-bottom-main w-100">
                ...
                <div class="card-body mb-2">
                    <img src="{{ empty($product->image) ? 'https://dummyimage.com/900x900/f8f9fc/919191.png&text=Sin+Imagen' : $product->image->url }}" class="img__principal"
                        alt="" id="principal_image">
                    <!-- <p class="text-center align-middle"> -->
                    <input type="text" class="invisible" name="principal_image" value="{{ empty($product->image) ? '' : $product->image->id}}" id="principal_image_field" hidden>
                    <!-- </p> -->
                </div>
                    ...
                    <p class="text-center align-middle">
                        ...
                        <div id="gallery_select" class="container__imgs grid">
                            @foreach($product->images as $image)
                                <div class='item container_item'>
                                    <img src="{{$image->url}}" class=thumbnail alt="{{$image->text_alter}}" data-src="{{$image->url}}">
                                    <input type=text id='gallery_images' name='gallery_photos[]' value="{{$image->id}}" hidden>
                                    <button id=deleteImg class=btnDeleteImg data-id="{{$image->id}}">
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="select_imgs" class="btn btn-info btn-icon-split btn-sm" data-toggle="modal"
                    data-target=".bd-example-modal-xl" data-dismiss="modal">
                    <span class="icon text-white-50">
...
<script>
    $(document).ready(function () {
        ...
        $.each($("#gallery_select > div"),function(ev,val){
    ...
    document.querySelector('#editProductForm').addEventListener('submit', function (e) {
        ...
        axios.put(this.action, {
            ...
            .then(function (response) {
                ...
                alert.innerHTML = (
                    `<div class="alert alert-success mt-2" role="alert"><strong> Muy bien.</strong> ${response.data.success}</div>`
                );
